...
class Lightbulb(VGroup):
    def __init__(self, radius=0.5, color=WHITE, fill_color=YELLOW, on=False):
        ...
        base = Union(base, base.copy().next_to(base, DOWN, buff=-radius/4))
        base = Union(base, Ellipse(width=radius/2,height=radius/4).move_to(base.get_critical_point(DOWN)), color=color, fill_color=GRAY, fill_opacity=1)
